...
public class BinPacking3DDemo {
    ...
    private static void basicDemo() {
        ...
        // 创建容器
        OptimizedContainer container = new OptimizedContainer(50, 40, 30, 1000);
        System.out.println("创建容器: " + container);
        
        // 创建一些测试盒子
        List<Box> boxes = Arrays.asList(
            new Box(1, 10, 8, 6, 50),   // 小盒子
            new Box(2, 15, 12, 10, 80), // 中盒子
            new Box(3, 20, 15, 12, 120), // 大盒子
            new Box(4, 8, 8, 8, 40),    // 正方体
            new Box(5, 25, 5, 8, 60)    // 长条形
        );
    ...
    private static void performanceComparison() {
        ...
        for (int size : testSizes) {
            ...
            // 优化算法测试
            OptimizedContainer optimizedContainer = new OptimizedContainer(100, 80, 60, 5000);
            long startTime = System.currentTimeMillis();
            ...
            if (size <= 20) {
                OptimizedContainer bruteContainer = new OptimizedContainer(100, 80, 60, 5000);
    ...
    private static void complexityAnalysis() {
        ...
        for (int size : sizes) {
            List<Box> boxes = generateRandomBoxes(size);
            OptimizedContainer container = new OptimizedContainer(80, 80, 80, 10000);
    ...
    private static void realWorldScenario() {
        ...
        double maxWeight = 28000;      // 28吨 = 28000公斤
        
        OptimizedContainer container = new OptimizedContainer(
            containerLength, containerWidth, containerHeight, maxWeight);
        
        System.out.println("集装箱规格: " + containerLength + "×" + containerWidth + "×" + containerHeight + " (分米)");
        System.out.println("最大载重: " + maxWeight + " 公斤\n");
        ...
        double costPerPackage = containerCost / placedCount;
        System.out.println("\n=== 经济效益 ===");
        System.out.println("集装箱运输成本: ¥" + containerCost);
        System.out.println("平均每包裹成本: ¥" + String.format("%.2f", costPerPackage));
        System.out.println("节省的包裹数: " + (packages.size() - placedCount));
    ...
    private static List<Box> generateRandomBoxes(int count) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            double weight = 5 + random.nextDouble() * 95;  // 5-100
    ...
    private static List<Box> generateRealisticPackages(int count) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            double weight = type[3] * (0.8 + random.nextDouble() * 0.4);
            
            packages.add(new Box(i, width, height, depth, weight));
        }
        
        // 按重量降序排序（重的先装）
        packages.sort((p1, p2) -> Double.compare(p2.getWeight(), p1.getWeight()));
